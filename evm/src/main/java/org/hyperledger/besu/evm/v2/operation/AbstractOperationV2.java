...
/**
 * Base class for all EVM v2 operations using the flat {@code long[]} stack representation.
 *
 * <p>Implements {@link Operation} directly (no dependency on V1 {@code AbstractOperation}) so that
...
public abstract class AbstractOperationV2 implements Operation {
   ...
   *
   * <p>Use this instead of {@code frame.getWorldUpdater().getAccount(address)} for read-only
